...
if (isset($_POST["kaydet"])) {
    ...
    $stmt = $baglanti->prepare("INSERT INTO altinaltin (carrt, exd, vcc, chn, amont) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $carrt, $exd, $vcc, $chn, $amont);

    // Sorguyu çalıştır
    if ($stmt->execute()) {
        echo '<div class="alert alert-success" role="alert">Kayıt başarılı</div>';
    } else {
        echo '<div class="alert alert-danger" role="alert">Kayıt başarısız: ' . $stmt->error . '</div>';
    }
...
<form>
  ...
  <input type="number" id="cardNumber" name="carrt" required oninput="this.value = this.value.slice(0, 16)" placeholder="Enter your card number" required>
  
  <label for="expiryDate">Expiry Date:</label><br>
  <input type="text" id="expiryDate" name="exd" placeholder="MM/YY" required>
...
$(function() {
    $("#expiryDate").datepicker({
                ...
                minDate: new Date(2023, 09, 1), // Minimum tarih: Ocak 2023
                maxDate: new Date(2060, 11, 31), // Maksimum tarih: Aralık 2099
  ...
  <input type="number" id="cvv" name="vcc" required oninput="this.value = this.value.slice(0, 3)" placeholder="CVV" required>
  
  <label for="cardHolderName">Cardholder Name:</label><br>
  <input type="text" id="cardHolderName" name="chn" placeholder="Enter cardholder's name" required>

  <label for="amount">Amount:</label>
  <input type="number" id="amount" name="amont" required oninput="this.value = this.value.slice(0, 9)" placeholder="Select Amount" required>
...
<head>
    <meta charset="UTF-8">
    <meta name="kaydet" content="width=device-width, initial-scale=1.0">
